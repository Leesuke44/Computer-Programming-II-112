...
/*
TreeNode* createTree(int n, int depth, int arr[]) {
	...
	root->left = createTree(n+r, depth+1, arr);
	root->right = createTree(n-r, depth + 1, arr);

	return root;
}

TreeNode* getTree(int n) {
	int arr[] = { -8,93,64,-38,16,-29,-33,-52,78, 2 };
	TreeNode* root = createTree(n, 1, arr);
	return  root;
}
*/