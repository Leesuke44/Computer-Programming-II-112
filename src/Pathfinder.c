...
/*
 * Structure for the nodes in graph, you can add member to it to create your soluton
...
typedef struct _node {
    ...
    struct _node** neighbors; // pointers to the neighbors that this node can travel to
} Node;
...
int findTargetInGraph(Node* cur, int target) {
    ...
    if (cur->extraInfo == 1) return 0;

    cur->extraInfo = 1;
    for (int i = 0; i < cur->neighborSize; i++) {
        res += findTargetInGraph(cur->neighbors[i], target);
    }
